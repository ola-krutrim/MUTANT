...
class AutoTikTokenizer:
    ...
    def _download_from_hf_hub(self, repo_name: str, token: str | None = None) -> str:
        """
        ...
        path = snapshot_download(repo_id=repo_name, allow_patterns=files_needed, token=token)
    ...
    @classmethod
    def from_pretrained(cls, tokenizer_name_or_path: str, token: str | None = None) -> tiktoken.Encoding:
        """
        Loads a pretrained tokenizer from the specified path or name and returns the TikToken encoding.
        ...
        if os.path.isfile(tokenizer_name_or_path):
            path = os.path.dirname(tokenizer_name_or_path)
            tokenizer = instance._read_json(tokenizer_name_or_path)
        elif os.path.isfile(os.path.join(tokenizer_name_or_path, 'tokenizer.json')):
            tokenizer = instance._read_json(os.path.join(tokenizer_name_or_path, 'tokenizer.json'))
        else:
            try:
                path = instance._download_from_hf_hub(tokenizer_name_or_path, token=token)
                tokenizer = instance._read_json(os.path.join(path, 'tokenizer.json'))
            except Exception as e:
                raise RuntimeError(
                    "Tokenizer could not be loaded from a local directory or from the Hugging Face Hub."
                ) from e
        ...
        tokenizer_type = instance._detect_tokenizer_type(tokenizer, {})
